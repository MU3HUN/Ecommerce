  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="姓名" prop="userName">
        <el-select v-model="queryParams.name" placeholder="请选择姓名" clearable @change="handleQuery">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['userinfo:info:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['userinfo:info:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['userinfo:info:logicDelete']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
          v-hasPermi="['userinfo:info:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="infoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="人员ID" align="center" prop="usersId" />
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="系统用户" prop="userId">
          <el-select v-model="form.userId" placeholder="请选择系统用户" filterable clearable style="width:100%">
            <el-option v-for="user in sysUserOptions" :key="user.userId" :label="`${user.userName} (${user.nickName})`"
              :value="user.userId">
            </el-option>
          </el-select>
        </el-form-item>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        userId: [
          { required: true, message: "系统用户不能为空", trigger: "change" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    getList() {
      ...
      listInfo(this.queryParams).then(response => {
        this.infoList = response.rows.sort((a, b) => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const usersIds = row.usersId ? [row.usersId] : this.ids;
      this.$modal.confirm(`确认删除选中的${usersIds.length}条数据？`).then(() => {
        return delInfo(usersIds.join(','));
      }).then(() => {
        this.getList();
